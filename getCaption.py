...
class MousePositionTracker(tk.Frame):
    ...
    def quit(self, event):
        aspect = 1.25
        x1 = int(self.start_x*1.25)
        y1 = int(self.start_y*1.25)
        x2 = int(self.end_x*1.25)
        y2 = int(self.end_y*1.25)

        #im = PIL.ImageGrab.grab().crop((self.start_x, self.start_y, self.end_x, self.end_y))
        im = PIL.ImageGrab.grab().crop((x1, y1, x2, y2))
        im.save(r"C:\Łukasz\wycinek.png")
        text = read_Text_From_Image(r"C:\Łukasz\wycinek.png")
        self.hide()  # Hide cross-hairs.
        self.reset()
        print("skopiowano: ", text)
        self.canvas.delete('all')
        check_call('echo ' + str(text).strip() + '|clip', shell=True)
...
class Application(tk.Frame):
    im = PIL.ImageGrab.grab(bbox=None, include_layered_windows=False, all_screens=True)
    width, height = im.size
    im = im.resize((int(width / 1.25), int(height / 1.25)))
    im.save(r"C:\Łukasz\test.png")
